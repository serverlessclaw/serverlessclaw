...
describe('puppeteer MCP server handler', () => {
  ...
  it('creates handler with correct server params', () => {
    expect(createMCPServerHandler).toHaveBeenCalledWith(
      expect.objectContaining({
        command: 'npx',
        args: expect.arrayContaining(['@kirkdeam/puppeteer-mcp-server']),
      })
    );
  });
  ...
  it('sets PUPPETEER_EXECUTABLE_PATH to /opt/chromium', () => {
    const callArgs = vi.mocked(createMCPServerHandler).mock.calls[0][0];
    expect(callArgs.env?.PUPPETEER_EXECUTABLE_PATH).toBe('/opt/chromium');
  });
});
