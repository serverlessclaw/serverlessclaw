...
export default function MemoryTable({ items, pruneAction, updateAction, bulkPruneAction }: MemoryTableProps) {
    ...
    <>
      <div className="flex justify-between items-center mb-4 px-2">
        <Typography variant="mono" color="muted" className="text-[10px] uppercase tracking-widest opacity-40">
          {selectedKeys.size > 0 ? `${selectedKeys.size} Records Selected` : 'Select records for bulk operations'}
        </Typography>
        {selectedKeys.size > 0 && (
          <Button
            variant="outline"
            ...
            icon={isBulkPruning ? <Loader2 size={12} className="animate-spin" /> : <Trash2 size={12} />}
              ...
              <tr className="border-b border-white/5 bg-white/[0.02]">
                <th className="pl-6 pr-2 py-4 w-10">
                  <button onClick={toggleSelectAll} className="text-white/20 hover:text-white transition-colors">
                    {selectedKeys.size === items.length && items.length > 0 ? <CheckSquare size={14} className="text-cyber-blue" /> : <Square size={14} />}
                  </button>
                </th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest text-white/40">Category</th>
                ...
                return (
                  <tr
                    ...
                    className={`hover:bg-white/[0.03] transition-colors cursor-pointer group ${
                      isSelected ? 'bg-cyber-blue/[0.05]' : (item.metadata?.priority && item.metadata.priority >= 8 ? 'bg-amber-500/[0.03]' : '')
                    }`}
                  >
