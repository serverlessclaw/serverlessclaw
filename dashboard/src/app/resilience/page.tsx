...
import { ShieldCheck, Activity, Clock, AlertTriangle, CheckCircle, Zap, Timer } from 'lucide-react';
        ...
        <div className="flex justify-center">
          <div className="relative">
            <ResilienceGauge
              ...
              label="Recovery"
              subtitle={
                recoveryState?.status === 'unhealthy'
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="text-cyber-green font-bold text-xs">
                  {health.deployCountToday !== undefined
                    ? `ACTIVE (${health.deployCountToday} deploys today)`
                    : 'ACTIVE'}
                </span>
              </div>
              <div className="flex justify-between items-center text-sm">
            ...
            {!isHealthy && (
              <div className="mt-8 p-4 bg-red-500/10 border border-red-500/30 rounded text-red-400 text-xs leading-relaxed italic space-y-2">
                <div className="font-bold">
                  CAUTION:{' '}
                  {health.message ||
                    'System health check falling below threshold. Investigate logs.'}
                </div>
                {health.details && (
                ...
                <div className="pt-2 text-[10px] opacity-60 not-italic">
                  If this is a local development environment, ensure the WebhookApi is reachable or
          ...
          <Card variant="solid" padding="lg" className="space-y-4 border-white/5">
            ...
            <div className="space-y-3">
              {logs.length > 0 ? (
                logs.map(
                  (
                    log: { timestamp?: number; key?: string; value?: string; content?: string },
                    idx: number
                  ) => (
                    <div
                      key={idx}
                ...
                <div className="h-64 flex flex-col items-center justify-center text-white/50 border border-dashed border-white/10 rounded-lg bg-white/[0.01]">
                  ...
                  <p className="text-xs font-mono uppercase tracking-widest">
                    No recovery events found // system_stable
                  </p>
                </div>
              )}
            </div>
