...
describe('SafetyTierEditor', () => {
  ...
  it('should display tier descriptions', () => {
    ...
    expect(screen.getByText('Isolated execution environment with strict boundaries.')).toBeInTheDocument();
    expect(screen.getByText('Full operational authority with guardrails and audit trail.')).toBeInTheDocument();
  });
  ...
  it('should call onTierChange when clicking Sandbox tier', () => {
    const onTierChange = vi.fn();
    render(<SafetyTierEditor {...defaultProps} currentTier="autonomous" onTierChange={onTierChange} />);
