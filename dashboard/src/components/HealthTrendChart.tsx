...
export default function HealthTrendChart({ currentScore }: HealthTrendChartProps) {
  ...
  return (
    <Card variant="glass" padding="lg" className="border-white/5 bg-black/40">
      <Typography variant="caption" weight="black" color="intel" uppercase className="tracking-[0.4em] mb-6 block">7-Day Cognitive Stability Trend</Typography>
      
      <div className="relative h-[160px] w-full">
        <svg viewBox={`0 0 ${width} ${height + padding * 2}`} className="w-full h-full overflow-visible">
          {/* Grid lines */}
          {[0, 25, 50, 75, 100].map((tick) => {
            ...
            return (
              <g key={tick}>
                ...
                <text x="0" y={y} fill="white" fillOpacity="0.2" fontSize="8" fontFamily="monospace" alignmentBaseline="middle">{tick}</text>
              </g>
            );
          })}
          ...
          {data.map((d, i) => {
            ...
            return (
              <g key={i} className="group/point">
                <circle cx={x} cy={y} r="3" fill="var(--cyber-blue)" className="transition-all duration-300 group-hover/point:r-5" />
                <text 
                  x={x} 
                  y={height + padding + 15} 
                  fill="white" 
